ALTER TRIGGER trVersion_update ON VRSION
	AFTER UPDATE
AS
BEGIN
	SET NOCOUNT ON;
...
SET @mod = CASE WHEN @referente_d != @referente_i THEN 'R'
				WHEN @revisor_d != @revisor_i OR (@revisor_d IS NULL AND @revisor_i IS NOT NULL) THEN 'V'
				WHEN @aprobador_d != @aprobador_i OR (@aprobador_d IS NULL AND @aprobador_i IS NOT NULL)THEN 'A'  
				END
